...
async def _stream_upstream(
    ...
    saw_filter = False
    buf = b""
    # ZCode treats the mere presence of ``tool_calls`` as a transition into a
    # tool-call block.  CodeBuddy emits ``tool_calls: []`` on ordinary
    # reasoning deltas; remove that empty marker while preserving real calls.
    normalize_buf = b""
            ...
            async with c.stream("POST", url, headers=headers, json=body) as r:
                ...
                async for chunk in r.aiter_bytes():
                    if chunk:
                        ...
                        _feed(chunk)
                        normalize_buf += chunk
                        # Normalize complete SSE lines.  Keeping a partial
                        # line buffered avoids corrupting JSON split across
                        # network reads.
                        out = bytearray()
                        while b"\n" in normalize_buf:
                            line, normalize_buf = normalize_buf.split(b"\n", 1)
                            if line.lstrip().startswith(b"data:"):
                                ...
                                try:
                                    ...
                                    if isinstance(obj, dict):
                                        ...
                                        line = prefix + b": " + json.dumps(obj, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
